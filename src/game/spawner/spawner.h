...
namespace spawner {
    ...
    uint32_t SpawnHomingShot(GameWorld& gameWorld, AssetManager& assMan, PhysicalInfo pi, uint32_t shooterId, uint32_t id = 0, bool predicted = false, uint32_t tick = 0);
    uint32_t SpawnShotFromNetwork(GameWorld& gameWorld, AssetManager& assMan, PhysicalInfo pi, uint32_t shooterId, uint32_t id, float age, bool isHoming = false);
