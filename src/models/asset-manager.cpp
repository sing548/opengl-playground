...
void AssetManager::ProcessNode(aiNode *node, const aiScene *scene, std::string& directory, const std::string& path)
{
	for (unsigned int i = 0; i < node->mNumMeshes; i++)
	{
        ...
        std::shared_ptr<Mesh> pt = std::make_shared<Mesh>(ProcessMesh(mesh, scene, directory));
		modelCache_[path].push_back(pt);
	}
...
unsigned int AssetManager::CreateBlackTexture()
{
    unsigned int textureID;
    glGenTextures(1, &textureID);
    glBindTexture(GL_TEXTURE_2D, textureID);
